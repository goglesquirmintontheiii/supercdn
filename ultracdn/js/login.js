...
function iter(i){if(i.startsWith("cookie"))
{ck=i.split("=")[1]
ck=ck.replace(" ","")
httpGet('/api/verify',{"Authorization":ck}).then(text=>{console.log(text);if(text!="None"){document.location="/"}else{document.cookie="";}}).catch(error=>{console.error(error);});}}
cks.forEach(iter)}

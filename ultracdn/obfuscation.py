...
def obfuscatev2(code):
    # Step 1: Generate a random number between 10 and 99
    random_num = random.randint(10, 99)

    # Step 2: Convert each character to its ASCII code and add the random number
    ascii_code = ''
    for char in code:
        ascii_code += str(ord(char) + random_num) + ','

    # Step 3: Reverse the ASCII codes and concatenate them
    reversed_code = ascii_code[::-1]

    # Step 4: Add the obfuscation function to the code
    obfuscation_function = f"""
        function base(num) {{
            return parseInt(num.toString().split('').reduce(function(prev, next) {{
                return (prev << 8) + parseInt(next)
            }}, ''), 10);
        }}

        function obfuscate() {{
            var code = [{reversed_code[:-1]}];
            return code.map(function(num) {{
                return String.fromCharCode(base(num) - {random_num});
            }}).join('');
        }}
    """

    # Step 5: Return the obfuscated code as a single string that can be run directly
    return f"""

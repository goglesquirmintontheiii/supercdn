...
TKB = (
    512 * 1024 * 1024
)  # TKB; Token bandwith; how much data can be POSTed using a token
...
cursor = conn.cursor()
cursor.execute(
    "CREATE TABLE IF NOT EXISTS users (username TEXT, password TEXT, email TEXT, capacity INTEGER)"
)
    ...
    for i in auth.keys():
        ...
        if cursor.fetchone() is None:
            cursor.execute(
                ...
                (i, auth[i]["password"], auth[i]["email"], auth[i]["capacity"]),
            )
        else:
            cursor.execute(
                "UPDATE users SET password=?, email=?, capacity=? WHERE username=?",
                (auth[i]["password"], auth[i]["email"], auth[i]["capacity"], i),
            )
    ...
    if len(args) == 2:
        ...
        for i in auth.keys():
            if auth[i]["password"] == pw and i == args[0]:
                if mkc:
                    cookie = (
                        "WARNING_SHARING_THIS_WILL_LET_OTHERS_LOGIN_TO_YOUR_ACCOUNT_"
                        + genstring(CKLN)
                    )
...
def fileop():
    ...
    if request.method == "GET":
        ...
        if os.path.exists(fp):
            ...
            return send_file(
                tempf, as_attachment=True, download_name=fp.split(os.path.sep)[-1]
            )
        else:

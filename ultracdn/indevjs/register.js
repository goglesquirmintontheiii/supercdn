...
function register() {
    ...
    remb = document.getElementById("rememberMe").value;
    agree = document.getElementById("agreeTos").value;
    if (user.length == 0 || pass.length == 0 || email.length == 0 || remb == false || agree == false) {
        return "";
    }
    httpPost('/api/register', { "username": user, "password": pass, "email":email })
        .then(text => {
            console.log(text);
            if (text != "false") {
                //document.cookie = "cookie=" + text + ";";
